...
export function registerLabelActions(rl: RunlinePluginAPI) {
  ...
  listAction(
    "label.list",
    "List labels (workspace + team-scoped).",
    "issueLabels",
    "IssueLabelFilter",
    LABEL_FIELDS,
  );
  ...
  rl.registerAction("label.create", {
    ...
    inputSchema: t.Object({
      ...
      teamId: t.Optional(
        t.String({
          description:
            "The team associated with the label. If omitted, the label is workspace-scoped",
        }),
      ),
      color: t.Optional(
        t.String({ description: "The color of the label (hex)" }),
      ),
      description: t.Optional(
        t.String({ description: "The description of the label" }),
      ),
      parentId: t.Optional(
        t.String({
      ...
      isGroup: t.Optional(
        t.Boolean({ description: "Whether the label is a group" }),
      ),
      retiredAt: t.Optional(
        t.String({
          description:
            "The time at which the label was retired (DateTime). Set to null to restore a retired label",
        }),
      ),
      id: t.Optional(
        t.String({
      ...
      replaceTeamLabels: t.Optional(
        t.Boolean({
          description:
            "Replace all team-specific labels with the same name with this newly created workspace label (default false)",
        }),
      ),
    }),
  ...
  rl.registerAction("label.update", {
    ...
    inputSchema: t.Object({
      ...
      parentId: t.Optional(
        t.String({ description: "The identifier of the parent label" }),
      ),
      isGroup: t.Optional(
        t.Boolean({ description: "Whether the label is a group" }),
      ),
      ...
      replaceTeamLabels: t.Optional(
        t.Boolean({
          description:
            "Replace all team-specific labels with the same name with this updated workspace label (default false)",
        }),
      ),
    }),
    async execute(input, ctx) {
      requireUnscoped(ctx, "label.update");
      const { id, replaceTeamLabels, ...fields } = input as Record<
        string,
        unknown
      >;
      const data = await gql(
