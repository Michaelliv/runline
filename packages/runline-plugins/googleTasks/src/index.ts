...
async function refreshAccessToken(ctx: Ctx): Promise<string> {
  const cfg = ctx.connection.config as unknown as GoogleTasksConfig;
  const { clientId, clientSecret, refreshToken } = cfg;
  if (!clientId || !clientSecret || !refreshToken) {
    throw new Error(
      "googleTasks: missing clientId/clientSecret/refreshToken. Run the Tasks OAuth helper to seed these.",
    );
  }
