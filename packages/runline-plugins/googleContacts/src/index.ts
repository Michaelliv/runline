...
async function refreshAccessToken(ctx: Ctx): Promise<string> {
  const cfg = ctx.connection.config as unknown as GoogleContactsConfig;
  const { clientId, clientSecret, refreshToken } = cfg;
  if (!clientId || !clientSecret || !refreshToken) {
    throw new Error(
      "googleContacts: missing clientId/clientSecret/refreshToken. Run the Contacts OAuth helper to seed these.",
    );
  }
