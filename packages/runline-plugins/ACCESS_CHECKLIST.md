...
| `shiftAtlas` | done | Annotated 35 actions covering the operational graph (`/v1/services/operational-graph`): 9 read (`summary`, `graph.get`, `stage.get`, `journey.get/list`, `actor.list`, `line.list`, `labels.get`, `change.list`) and 26 write (actor/line/journey/stage/task/assignment/relation/binding/evidence create/update/delete plus `labels.update`). |
...
| `shiftObjects` | done | Annotated 8 actions (split out of the former shiftLabs plugin): 5 read (`object.get/list/links/download/bucket` — download mints a signed grant and optionally saves to disk) and 3 write (`object.upload/attach/archive`). |
| `shiftPages` | done | Annotated 11 actions: 5 read (`page.get/list/shares/renderUrl/vexArtifacts`) and 6 write (`page.create/update/publish/archive/share/revokeShare`). |
| `shiftTranscription` | done | Annotated 7 actions (split out of the former shiftLabs plugin): 4 read (`transcription.job.get/list`, `transcription.artifact.list`, `transcription.transcript`) and 3 write (`transcription.transcribe`, `transcription.job.cancel`, `transcription.job.delete`). |
| `shiftWork` | done | Annotated 29 actions: 15 read (project/issue/issueView get/list/listPage, dependency and event timelines, view execution) and 14 write (project/issue/issueView create/update/delete, `issue.report`, lifecycle transitions `issue.resolve/reopen/close`, comments, dependency add/remove). |
